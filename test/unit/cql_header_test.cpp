...
BOOST_AUTO_TEST_CASE(getters_and_constructor)
{
    ...
    cql::cql_header_t header(CQL_VERSION_1_REQUEST, CQL_FLAG_NOFLAG, 1, CQL_OPCODE_READY, 5);
...
{
    cql::cql_header_t header(CQL_VERSION_1_REQUEST, CQL_FLAG_NOFLAG, 0, CQL_OPCODE_READY, 5);
    cql::cql_error_t err;
    header.prepare(&err);
...
BOOST_AUTO_TEST_CASE(serialization_round_trip)
{
    ...
    BOOST_CHECK_EQUAL(1, header.stream());
    BOOST_CHECK_EQUAL(CQL_OPCODE_READY, header.opcode());
    BOOST_CHECK_EQUAL(5, header.length());
}
