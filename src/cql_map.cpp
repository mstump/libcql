...
{
    cql_short_t count = 0;
    cql_byte_t* _pos = cql::decode_short(_start, count);
    _keys.reserve(count);
...
bool
cql::cql_map_t::get_value_double(
    size_t i,
    double& output) const
{
    if (i > _values.size()) {
...
bool
cql::cql_map_t::get_value_bigint(
    size_t i,
    cql_bigint_t& output) const
{
    if (i > _values.size()) {
...
bool
cql::cql_map_t::get_value_string(
    size_t i,
    std::string& output) const
{
    cql_byte_t* data = 0;
...
std::string
cql::cql_map_t::str() const
{
    return "map";
}

cql_int_t
cql::cql_map_t::key_type() const
{
    return _key_type;
}

const std::string&
cql::cql_map_t::key_custom_class() const
{
    return _key_custom_class;
}

cql_int_t
cql::cql_map_t::value_type() const
{
    return _value_type;
}

const std::string&
cql::cql_map_t::value_custom_class() const
{
    return _value_custom_class;
}

size_t
cql::cql_map_t::size() const
{
    return _keys.size();
}
