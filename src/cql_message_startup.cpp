...
cql::cql_message_startup_t::buffer()
{
    return _buffer;
}

void
cql::cql_message_startup_t::compression(
    const std::string& c)
{
    _compression = c;
}
...
void
cql::cql_message_startup_t::version(
    const std::string& v)
{
    _version = v;
}
...
bool
cql::cql_message_startup_t::consume(
    cql::cql_error_t*)
{
...
bool
cql::cql_message_startup_t::prepare(
    cql::cql_error_t*)
{
    std::map<std::string, std::string> startup;
    ...
    if (!_version.empty()) {
        ...
        size += strlen(CQL_VERSION) + _version.size() + (2* sizeof(cql_short_t));
    }

    if (!_compression.empty()) {
        startup.insert(std::pair<std::string, std::string>(CQL_COMPRESSION, _compression));
        size += strlen(CQL_COMPRESSION) + _compression.size() + (2* sizeof(cql_short_t));
    }
