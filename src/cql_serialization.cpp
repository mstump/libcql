...
void
cql::encode_bool(std::vector<cql_byte_t>& output,
                 const bool value)
{
    output.resize(1, 0);
    if (value) {
...
bool
cql::decode_bool(const std::vector<cql_byte_t>& input)
{
    return input[0];
}
...
{
    cql_short_t s = htons(value);
    output.write(reinterpret_cast<char*>(&s), sizeof(s));
    return output;
}

void
cql::encode_short(std::vector<cql_byte_t>& output,
                  const cql_short_t value)
{
    cql_short_t s = htons(value);
    output.resize(sizeof(s));
    const cql_byte_t* it = reinterpret_cast<cql_byte_t*>(&s);
...
cql_short_t
cql::decode_short(const std::vector<cql_byte_t>& input)
{
    return ntohs(*(reinterpret_cast<const cql_short_t*>(&input[0])));
}

cql_byte_t*
cql::decode_short(cql_byte_t* input,
                  cql_short_t& value)
{
    value = ntohs(*(reinterpret_cast<const cql_short_t*>(input)));
    return input + sizeof(cql_short_t);
}
    ...
    const cql_int_t          value)
{
    ...
    const cql_byte_t* it = reinterpret_cast<cql_byte_t*>(&l);
...
cql_int_t
cql::decode_int(
    const std::vector<cql_byte_t>& input)
{
...
cql::decode_int(
    ...
    cql_int_t&  output)
{
    output = ntohl(*(reinterpret_cast<const cql_int_t*>(input)));
    return input + sizeof(cql_int_t);
}
    ...
    const double             value)
{
    ...
    const cql_byte_t* it = reinterpret_cast<cql_byte_t*>(&d);
...
{
    cql_bigint_t d = ntoh<cql_bigint_t>(value);
    output.write(reinterpret_cast<char*>(&d), sizeof(d));
    return output;
}
...
cql::decode_bigint(
    const std::vector<cql_byte_t>& input)
{
    return ntoh<cql_bigint_t>(*(reinterpret_cast<const cql_bigint_t*>(&input[0])));
}
...
cql::decode_bigint(
    ...
    cql_bigint_t& output)
{
    output = ntoh<cql_bigint_t>(*(reinterpret_cast<const cql_bigint_t*>(input)));
    return input + sizeof(cql_bigint_t);
}
...
cql::encode_option(
    ...
    const std::string& value)
{
    cql::encode_short(output, reinterpret_cast<cql_short_t&>(id));
